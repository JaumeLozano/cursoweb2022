...
if (isset($_POST['submit'])) {
    ...
    if (!empty($nombre)) {
        ...
        $erroresnombre = 'Por favor escribe un nombre <br>';
    }
    if (!empty($email)) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {//Filtro de "validación" del correo
            $erroresemail = 'Por favor escribe un correo válido <br>';
        } else {
            echo 'Tu correo es ' . $email . '<br>';
        }
    } else {
        $erroresemail = 'Por favor escribe un correo <br>';
    }
}
...
<body>
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post"> 
        ...
        <div class="error"><?php echo $erroresnombre; ?></div>
        <br>
        <input type="text" name="email" placeholder="Email" id=""> <!--Tipo normal type="email"-->
        <div class="error"><?php echo $erroresemail; ?></div>
        <br>
